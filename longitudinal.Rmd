...
I usually start with a very simple Bayesian analysis because if something is awry, I'll notice it faster with a simple example. The code below implements a simple beat-binomial model.
...
Our longitudinal setting has two measurements: pre and post. So strip the last three columns from the matrix Y. There is one within subject factor, time, which is represents by X1. There is a between subjects factor, treatment group, which is represented by X2.                        
...
dat$Y <- t(dat$Y)
init <- list(alpha = rep(150, 30),
      beta1 = 50, beta2 = 20, beta3 = 5,
      alpha.c = 150,
      tau.c = 1, alpha.tau = 1) 
...
fnm <- "jags_lon1.txt"
mon <- c("alpha.c", "beta1", "beta2", "beta3")
...
Okay, so far, so good. The research hypothesis, related to a non-inferiority question, is that the probability that the treatment group is not more than 2.5 units worse than the control has to be at least 80%. There are several ways to approach this, but one simple way is just to count the number of times that beta3 is less than or equal to 2.5.

```{r counting_beta3}
sum(unlist(out$lon1[ ,4])<=2.5)
```

Since 970 of the 1000 runs are less tan or 